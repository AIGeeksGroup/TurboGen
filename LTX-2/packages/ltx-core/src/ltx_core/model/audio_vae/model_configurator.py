...
class VocoderConfigurator(ModelConfigurator[Vocoder]):
    ...
    def from_config(cls: type[Vocoder], config: dict) -> Vocoder:
        ...
        if isinstance(config.get("vocoder"), dict):
            nested_config = dict(config["vocoder"])
            flat_overrides = {key: value for key, value in config.items() if key not in {"vocoder", "bwe"}}
            nested_config.update(flat_overrides)
        ...
        return Vocoder(
            ...
            resblock=resblock,
            output_sample_rate=config.get("output_sample_rate", config.get("output_sampling_rate", 24000)),
