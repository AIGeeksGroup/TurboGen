...
class PerChannelStatistics(nn.Module):
    ...
    def materialize_missing_buffers(self, device: torch.device, dtype: torch.dtype | None = None) -> None:
        latent_channels = self.get_buffer("std-of-means").shape[0]
        dtype = dtype or self.get_buffer("std-of-means").dtype
        defaults = {
            "std-of-means": torch.ones(latent_channels, device=device, dtype=dtype),
            "mean-of-means": torch.zeros(latent_channels, device=device, dtype=dtype),
            "mean-of-stds": torch.ones(latent_channels, device=device, dtype=dtype),
            "mean-of-stds_over_std-of-means": torch.ones(latent_channels, device=device, dtype=dtype),
            "channel": torch.arange(latent_channels, device=device, dtype=dtype),
        }
        for name, value in defaults.items():
            if str(self.get_buffer(name).device) == "meta":
