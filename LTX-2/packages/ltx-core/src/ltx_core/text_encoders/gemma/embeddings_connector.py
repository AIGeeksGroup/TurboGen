...
class Embeddings1DConnectorConfigurator(ModelConfigurator[Embeddings1DConnector]):
    @classmethod
    def from_config(
        cls: type[Embeddings1DConnector],
        config: dict,
        *,
        prefix: str = "",
        ...
        pe_max_pos = config.get(f"{prefix}connector_positional_embedding_max_pos", config.get("connector_positional_embedding_max_pos", [1]))
        attention_head_dim = config.get(f"{prefix}connector_attention_head_dim", 128)
        num_attention_heads = config.get(f"{prefix}connector_num_attention_heads", 30)
        num_layers = config.get(f"{prefix}connector_num_layers", config.get("connector_num_layers", 2))
        num_learnable_registers = config.get(
            f"{prefix}connector_num_learnable_registers",
            config.get("connector_num_learnable_registers", 128),
        )
