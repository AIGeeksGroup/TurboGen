...
class AVGemmaTextEncoderModelConfigurator(ModelConfigurator[AVGemmaTextEncoderModel]):
    ...
    def from_config(cls: type["AVGemmaTextEncoderModel"], config: dict) -> "AVGemmaTextEncoderModel":
        ...
        audio_embeddings_connector = Embeddings1DConnectorConfigurator.from_config(config, prefix="audio_")
...
def create_and_populate(module: AVGemmaTextEncoderModel) -> AVGemmaTextEncoderModel:
    def register_or_set_buffer(target, name: str, value: torch.Tensor):
        if name in target._buffers:
            target._buffers[name] = value
        else:
    ...
    config.rope_theta = getattr(config, "rope_theta", None) or 1000000
    base = getattr(config, "rope_local_base_freq", None) or 10000
    local_rope_freqs = 1.0 / (base ** (torch.arange(0, dim, 2, dtype=torch.int64).to(dtype=torch.float) / dim))
    raw_rope_scaling = getattr(config, "rope_scaling", {}) or {}
    if isinstance(raw_rope_scaling, dict):
        rope_scaling = dict(raw_rope_scaling)
    else:
        rope_scaling = dict(getattr(raw_rope_scaling, "__dict__", {}))
    rope_type = rope_scaling.get("rope_type") or rope_scaling.get("type") or "linear"
    rope_scaling.setdefault("rope_type", rope_type)
    rope_scaling.setdefault("type", rope_type)
    rope_scaling["factor"] = rope_scaling.get("factor") or 8.0
    config.rope_scaling = rope_scaling
    inv_freqs, _ = ROPE_INIT_FUNCTIONS[rope_type](config)

    if hasattr(v_model.embeddings, "position_ids"):
        positions_length = len(v_model.embeddings.position_ids[0])
    else:
        positions_length = v_model.embeddings.position_embedding.num_embeddings
    position_ids = torch.arange(positions_length, dtype=torch.long, device="cpu").unsqueeze(0)
    register_or_set_buffer(v_model.embeddings, "position_ids", position_ids)
    embed_scale = torch.tensor(model.config.text_config.hidden_size**0.5, device="cpu")
    register_or_set_buffer(l_model.embed_tokens, "embed_scale", embed_scale)
    if hasattr(l_model, "rotary_emb_local"):
        register_or_set_buffer(l_model.rotary_emb_local, "inv_freq", local_rope_freqs)
    register_or_set_buffer(l_model.rotary_emb, "inv_freq", inv_freqs)
