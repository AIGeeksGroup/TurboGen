...
class SingleGPUModelBuilder(Generic[ModelType], ModelBuilderProtocol[ModelType], LoRAAdaptableProtocol):
    ...
    def _return_model(self, meta_model: ModelType, device: torch.device) -> ModelType:
        for module in meta_model.modules():
            materialize_missing_buffers = getattr(module, "materialize_missing_buffers", None)
            if materialize_missing_buffers is not None:
                materialize_missing_buffers(device)
