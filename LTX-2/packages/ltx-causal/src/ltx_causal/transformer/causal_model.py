...
class CausalLTXModelConfig:
    ...
    audio_caption_channels: Optional[int] = None  # Audio text encoder output dim; defaults to caption_channels
...
class CausalLTXModel(nn.Module):
    ...
    Module name mapping to original LTXModel:
        ...
        av_ca_audio_scale_shift_adaln_single 鈫?AdaLayerNormSingle(2048, coefficient=4)
        av_ca_a2v_gate_adaln_single          鈫?AdaLayerNormSingle(4096, coefficient=1)
        av_ca_v2a_gate_adaln_single          鈫?AdaLayerNormSingle(2048, coefficient=1)
        transformer_blocks      鈫?ModuleList of CausalAVTransformerBlock
        scale_shift_table       鈫?Parameter([2, 4096])
        norm_out                鈫?LayerNorm(4096, elementwise_affine=False)
        proj_out                鈫?nn.Linear(4096, 128)
        audio_scale_shift_table 鈫?Parameter([2, 2048])
        audio_norm_out          鈫?LayerNorm(2048, elementwise_affine=False)
        audio_proj_out          鈫?nn.Linear(2048, 128)
    """
    ...
    def __init__(self, config: CausalLTXModelConfig):
        ...
        self.audio_caption_projection = (
            PixArtAlphaTextProjection(
                in_features=config.audio_caption_channels or config.caption_channels,
    ...
    def forward(
        ...
        # === Patch Embedding ===
        # Video: [B, F, C, H, W] 鈫?patchify 鈫?[B, T, C] 鈫?project 鈫?[B, T, D]
