...
class LTX2DMD(nn.Module):
    ...
    def __init__(self, args, device: torch.device):
        ...
        # Pre-compute sigma lookup table for random timestep 鈫?sigma conversion.
        ...
        # full_sigmas goes from ~1.0 (noise) to 0.0 (clean), same order as timesteps 1000鈫?
        # We need sigma_lookup[t] where t=0 鈫?sigma=0 (clean) and t=1000 鈫?sigma鈮? (noise)
        # full_sigmas is ordered: sigma[0]=high (noise), sigma[-1]=0 (clean)
        # So sigma_lookup[t] = full_sigmas[1000 - t] maps t=1000鈫抐ull_sigmas[0], t=0鈫抐ull_sigmas[1000]
        self.register_buffer(
            'sigma_lookup',
            full_sigmas.flip(0),  # Reverse so index 0=clean(蟽鈮?), index 1000=noise(蟽鈮?)
        )
        ...
        if self.dmd_latent_mode == "teacher_denoise":
            ...
            # teacher_sigmas[0] 鈮?1.0 (noise), teacher_sigmas[-1] = 0.0 (clean)
    ...
    def init_models(self):
        ...
        text_encoder_device = (
            torch.device("cpu")
            if getattr(args, "text_encoder_device", "cuda") == "cpu"
            else target_torch_device
        )
        ...
        vae_device = (
            torch.device("cpu")
            if getattr(args, "no_visualize", False) or not getattr(args, "benchmark_enabled", True)
            else target_torch_device
        )
    ...
    def _run_self_forcing_rollout(
        ...
        *,
        ...
        鈥?no multi-backward, fully FSDP-compatible.
        ...
        # Output tensors 鈥?all blocks write via in-place slice assignment,
        ...
        for b_idx, block in enumerate(rollout_blocks):
            ...
            # Write to output tensor 鈥?autograd retained via slice assignment.
    ...
    def _get_noisy_latent_via_teacher_denoise(
        ...
        falls in the pre-computed teacher_sigmas schedule 鈥?no randomization.
        ...
        teacher_sigmas = self.teacher_sigmas  # [N+1], descending: [0]鈮?.0, [-1]=0.0
    ...
    def _consistency_backward_simulation(
        ...
        active for the subsequent gradient-enabled forward pass 鈥?essential
    ...
    def _generator_loss_self_forcing_two_pass(
        ...
        *,
        ...
        Peak activation memory 鈮?ONE block's exit-step forward. The two-pass
        design serializes across blocks so we never hold >1 block's autograd
        graph at a time:

          Pass 1: full no_grad rollout 鈫?rollout_video_detached (no activations).
          ...
          Pass 2: per block, PAIRED forward鈫抣oss鈫抌ackward鈫抐ree.
        ...
        (Pass 1's clean x0) 鈥?this is detached regardless of Pass 2 grad state
        and keeps the KV cache bit-identical between passes.

        Cost: roughly 2脳 Self-Forcing's rollout forward count (Pass 1 full
        rollout + Pass 2 denoising loops). Backward cost is unchanged
        (backward fires once per block, vs single backward over the whole
        video). Wall clock ~1.5-2脳 K=1, in exchange for N脳 per-iter gradient
        ...
        # the whole video, so denom = B 脳 F_v (equivalent to Self-Forcing's
        ...
        # RNG-restored 鈫?identical exit indices as Pass 1.
        ...
        # denominator (B 脳 F_v / B 脳 F_a), so summing across blocks yields the
        ...
        # Backward already ran per block 鈥?return no-grad scalar tensor so the
