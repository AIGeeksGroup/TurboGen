...
class LTX2ODEPairGenerator(nn.Module):
    ...
    def generate_trajectory(
        ...
        def move_conditioning_to_device(conditioning: Dict[str, torch.Tensor] | None):
            if conditioning is None:
                return None
            return {
                key: value.to(device=device, dtype=dtype, non_blocking=True) if torch.is_tensor(value) else value
                for key, value in conditioning.items()
            }

        conditional_dict = move_conditioning_to_device(conditional_dict)
        unconditional_dict = move_conditioning_to_device(unconditional_dict)
